...
public class MyBaiduEngine extends RamCrawler {
	...
	private BaiduExcute baiduExcute;
	

	public MyBaiduEngine(String keyword, int maxPageNum,BaiduExcute baiduExcute) {
		this.baiduExcute = baiduExcute;
...
//        //爬取搜索关键字“网络爬虫”结果的前3页
//		//中国
//		MyBaiduEngine crawler = new MyBaiduEngine("韩国", 1,null);
//		//设置爬取深度
//		crawler.start(3);
...
//							+ baiduExcute.getPatten());
					
					/** 废弃 **/
//					if(baiduExcute!=null){
//						Elements types = doc.select(baiduExcute.getPatten());//
//						if (types.size() > 0) {
//							baiduExcute.excute(types.get(0));
//						}
//					}
					
					
					//beList
					if(baiduExcute!=null&&baiduExcute.getPatten().size()>0){
						List<Element> typeList = new ArrayList<Element>();
						for (int i = 0; i < baiduExcute.getPatten().size(); i++) {
							Elements types = doc.select(baiduExcute.getPatten().get(i));//
							if (types.size() > 0) {
								typeList.add(types.get(0));
							}else{
								typeList.add(null);
							}
						}
						baiduExcute.excute(typeList);
					}
