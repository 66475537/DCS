...
public class MyBaiduEngine extends RamCrawler {
  ...
  //当前页面的depth为x，则从当前页面解析的后续任务的depth为x+1
	...
	public static void main(String[] args) throws Exception {
        //爬取搜索关键字“网络爬虫”结果的前3页
		//中国
		MyBaiduEngine crawler = new MyBaiduEngine("德国", 1);
		//设置爬取深度
		crawler.start(3);
	}
...
//			System.out.println(line);
			if(page.getDoc().title().contains("_百度百科")&&page.getDoc().title().indexOf("（")>0){
...
//                Elements types = doc.select(baiduExcute.getPatten());//
//              if(types.size()>1){
//            	  baiduExcute.excute(types.get(0).text());
//              }
                
                //人口数量
              Elements types = doc.select("dt:matchesOwn(国花) ~ dd");//
              if(types.size()>0){
//              	System.out.println(types.get(0).attr("src"));
            	  System.out.println(types.get(0).text());
                
//                DownloadImage.download(types.get(0).attr("src"), "test.jpg", "D:\\DCS_IMGS");
//              	new DemoImageCrawler(, "D:\\DCS_IMGS");
//              	dc.
              }
              
                
//                baiduExcute.excute(this.);
                
//                Elements types = doc.select("dt:matchesOwn(中文名称) ~ dd");//
//                if(types.size()>1){
//                	System.out.println(types.get(0).text());
//                }
//                Elements types1 = doc.select("dt:matchesOwn(外文名称) ~ dd");//
//                if(types1.size()>1){
//                	System.out.println(types1.get(0).text());
//                }
//                
//                Elements  types2 = doc.select("dt:matchesOwn(别) ~ dd");//
//                if(types2.size()>1){
//                	System.out.println(types2.get(0).text());
//                }
//                
//                types2 = doc.select("dt:matchesOwn(行政区类别) ~ dd");//
//                if(types2.size()>1){
//                	System.out.println(types2.get(0).text());
//                }
//                
//                types2 = doc.select("dt:matchesOwn(所属地区) ~ dd");//
//                if(types2.size()>1){
//                	System.out.println(types2.get(0).text());
//                }
//                
//                types2 = doc.select("dt:matchesOwn(电话区号) ~ dd");//
//                if(types2.size()>1){
//                	System.out.println(types2.get(0).text());
//                }
//                
//                types2 = doc.select("dt:matchesOwn(邮政区码) ~ dd");//
//                if(types2.size()>1){
//                	System.out.println(types2.get(0).text());
//                }
...
//                Elements types=doc.select("div[class='catalog-list column-4']");
